...
def cap(cid, subject, scope, *, parent=None, expires=200, uses=4, delegable=False):
    return CapabilityContract.build(capability_id=cid, capability_type="network.connect_domain", subject_id=subject, issuer_id="human:owner" if parent is None else "agent:a", scope=scope, issued_at_unix=10, not_before_unix=10, expires_at_unix=expires, max_uses=uses, delegable=delegable, parent_capability_id=parent, policy_sha256=P).as_document()
...
class MultiAgentTests(unittest.TestCase):
    def setUp(self):
        ...
        self.g.admit_root(cap("root","agent:a",self.scope,delegable=True),at_unix=20)
    def test_narrow_delegation_passes(self):
        r=self.g.delegate(cap("child","agent:b",{"domains":["api.example.com"],"protocols":["https"],"ports":[443]},parent="root",expires=150,uses=2),at_unix=30)
    ...
    def test_parent_revoke_revokes_child(self):
        ...
        out=self.g.revoke_tree("root",at_unix=40)
        self.assertEqual(len(out),2); self.assertTrue(all(x["decision"]=="ALLOW" for x in out))
    def test_child_use_after_parent_revoke_is_blocked(self):
        self.g.delegate(cap("child","agent:b",{"domains":["api.example.com"],"protocols":["https"],"ports":[443]},parent="root",expires=150,uses=2),at_unix=30)
        self.g.revoke_tree("root",at_unix=40)
        r=self.g.broker.authorize(subject_id="agent:b",capability_type="network.connect_domain",policy_sha256=P,requested_scope={"domains":["api.example.com"],"protocols":["https"],"ports":[443]},action={"call":"x"},at_unix=50)
        self.assertEqual(r["decision"],"BLOCK")
    def test_collective_escalation_and_replay_shape(self):
        e1=event(1,"package.install","agent:a","0"*64)
        e2=event(2,"network.connect_domain","agent:b",e1.event_sha256)
        e3=event(3,"credential.access","agent:a",e2.event_sha256,0,1)
        e4=event(4,"process.spawn_child","agent:b",e3.event_sha256,1,2)
        r1=self.g.analyze_collective((e1,e2,e3,e4)); r2=self.g.analyze_collective((e1,e2,e3,e4))
        self.assertEqual(r1["phase3"]["decision"],"CONTAIN"); self.assertEqual(r1["receipt_sha256"],r2["receipt_sha256"]); self.assertEqual(r1["subjects"],["agent:a","agent:b"])
