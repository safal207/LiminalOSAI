"""Host-bound process-session supervision for active LiminalOS containment.

This is the concrete session-control layer. It tracks opaque execution-session
bindings and invokes trusted host callbacks to inspect, freeze, terminate and
verify those sessions. It never exposes raw argv, environment variables,
workspace paths or host process identifiers to the model-facing control plane.

Fine-grained root/descendant proof is a separate optional profile implemented by
``sdk.liminal_process_lineage``.
"""
...
FREEZE_SCHEMA = "liminal-process-tree-freeze-receipt-v0.1"
ZERO_SHA256 = "0" * 64
AUTHORITY = {
    ...
    "separate_freeze_and_quiesce": True,
...
class ProcessTreeSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        self._freeze_receipts: list[dict[str, Any]] = []
    ...
    def freeze_all(self) -> dict[str, Any]:
        """Freeze active governed sessions without terminating them.

        This is the Phase-4 FREEZE hook. Termination is deliberately excluded so
        capability revocation can happen before quiescence.
        """
        candidates = tuple(
            self._sessions[key]
        ...
        for binding in candidates:
            ...
            try:
                ...
                failures.append(f"inspect_before:{session_hash}:{type(exc).__name__}")
            ...
            try:
                ...
                if not after.exists:
                    ...
                    failures.append(f"still_running:{session_hash}")
                    self._set_status(binding.session_id, "ERROR")
                else:
                    frozen_count += 1
                    self._set_status(binding.session_id, "FROZEN")
            except Exception as exc:
                failures.append(f"freeze:{session_hash}:{type(exc).__name__}")
                self._set_status(binding.session_id, "ERROR")
        ...
        body = {
            ...
            "before_root_sha256": canonical_sha256(before_docs),
            "after_root_sha256": canonical_sha256(after_docs),
            "authority_sha256": canonical_sha256(AUTHORITY),
        }
        receipt = {**body, "receipt_sha256": canonical_sha256(body)}
        self._freeze_receipts.append(receipt)
        return receipt

    def quiesce_session(self, session_id: str, *, incident_id: str) -> dict[str, Any]:
        """Emergency exact-session cleanup used by timeout/adapter failure paths."""
        _validate_id(incident_id, "incident_id")
        binding = self._get(session_id)
        return self._quiesce((binding,), incident_id=incident_id, allow_emergency_freeze=True)

    def quiesce_all(self, *, incident_id: str) -> dict[str, Any]:
        """Post-revocation termination path for Phase 4.

        Any still-running session is a failure: this method will not silently
        perform the missing FREEZE stage after revocation.
        """
        _validate_id(incident_id, "incident_id")
        candidates = tuple(
            self._sessions[key]
        ...
        return self._quiesce(candidates, incident_id=incident_id, allow_emergency_freeze=False)

    def containment_freeze_hook(self) -> Callable[[], None]:
        def hook() -> None:
            receipt = self.freeze_all()
            if receipt["failure_codes"]:
                raise ProcessTreeError("active containment could not freeze every execution session")
        return hook

    def containment_quiesce_hook(self, *, incident_id: str) -> Callable[[], Mapping[str, Any]]:
        def hook() -> Mapping[str, Any]:
            return self.quiesce_all(incident_id=incident_id)
        return hook
    ...
    def freeze_receipts(self) -> tuple[dict[str, Any], ...]:
        return tuple(dict(item) for item in self._freeze_receipts)
    ...
    def _quiesce(
        ...
        *,
        ...
        for binding in bindings:
            ...
            active_before += 1
            if before.running:
                if not allow_emergency_freeze:
                    failure_codes.append(f"not_frozen:{session_hash}")
                    self._set_status(binding.session_id, "ERROR")
                    survivors += 1
                    after_docs.append(before.safe_body())
                    continue
                try:
                    ...
                    before = self._inspect(binding.session_id)
                    if before.running:
                        raise ProcessTreeError("emergency freeze left session running")
                except Exception as exc:
        ...
        base = ProcessTreeContainmentReceipt(
            ...
            session_set_sha256=canonical_sha256(session_safe),
            before_root_sha256=canonical_sha256(before_docs),
            after_root_sha256=canonical_sha256(after_docs),
...
def verify_receipt(document: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in ("session_count", "active_before_count", "terminated_count", "already_absent_count", "survivor_count"):
...
__all__ = [
    "AUTHORITY", "FREEZE_SCHEMA", "ProcessTreeContainmentReceipt", "ProcessTreeError",
    "ProcessTreeObservation", "ProcessTreeSupervisor", "SCHEMA", "SessionBinding", "ZERO_SHA256",
    "verify_receipt",
]
